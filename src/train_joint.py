...
def joint_train(config):
    ...
    img_model = torch.load(config['img_model'], map_location=torch.device('mps'))  # tsx pre-trained model for sar amplitude img. (gpu load)
    ...
    if config['pretrained_model']:
        net_joint.load_state_dict(torch.load(config['pretrained_model'], map_location=torch.device('mps')))
    ...
    for epoch in range(epoch_num):
        for data in dataloader['train']:
            ...
            bbox_data = data['bbox'].to(device)
            label_output, bbox_output = net_joint(spe_data, img_data)
            label_loss = loss_func(label_output, labels)
            bb_loss = F.l1_loss(bbox_output, bbox_data, reduction="none").sum(1)
            bb_loss = bb_loss.sum()
            loss_weight_bb = torch.tensor(loss_weight)
            loss_weight_bb = loss_weight_bb.unsqueeze(0).unsqueeze(-1)

            # Calculate the L1 loss for bounding box regression
            # Apply class weighting
            bb_loss = bb_loss * loss_weight_bb

            # Calculate the mean of the weighted bounding box loss
            bb_loss = torch.mean(bb_loss)
            loss = label_loss + bb_loss / 1000
            loss.backward()
            optimizer.step()
            i += 1
        ...
        with torch.no_grad():
            for j in range(len(dataloader['val'])):
                ...
                val_output_class, val_output_bb = net_joint(val_spe, val_img)

                # val_loss = loss_func(val_output, val_label)
                _, pred = torch.Tensor.max(val_output_class, 1)
                acc_num += torch.sum(torch.squeeze(pred) == val_label.data).float()
                data_num += val_label.size()[0]
                val_label_loss = loss_func(val_output_class, val_label)
                val_bb_loss = F.l1_loss(val_output_bb, val_bb, reduction="none").sum(1)
                val_bb_loss = val_bb_loss.sum()
                loss = val_label_loss + val_bb_loss/1000
                val_loss += loss.item()
